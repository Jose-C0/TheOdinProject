...
function setAc() {
    ...
    str = 0;
    display.textContent = "";
    if (score != "💀 there was an error!") ans_display.textContent = "Ans = " + ans;
...
function convertStringToNumbers(str) {
    ...
    for (const iterator of newArray) {
        ...
        if (typeof iterator === "string" && !listValuesOperators.includes(iterator)) {
            ...
            let exponent = Number(stringSplit.slice(posAfterX, stringSplit.length).join(""));
...
function finalScore(string) {
    ...
    for (let index = 0; index < string.length + 1; index++) {
        ...
        if (posDivide != -1) {
            ...
            if (r === "💀 there was an error!") { return r }
            continue;
        }
